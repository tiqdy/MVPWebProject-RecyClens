...
<style>
    ...
    @media only screen and (max-width: 768px) {{
        ...
        /* Result Cards - Constrained Width & Centered */
        .result-card-success, .result-card-error {{
            width: 90%;
            margin: 20px auto;
            padding: 15px;
        }}
        ...
        /* Read More Button - Constrained Width & Centered */
        div.stButton > button {{
            width: auto !important;
            margin: 0 auto;
            display: block;
...
# 5. FIXED HEADER (NO INDENTATION TO PREVENT CODE BLOCK)
st.markdown(f"""
...
with c2:
    st.markdown(f'<div class="section-header">Hi, {current_user}! Ready to recycle?</div>', unsafe_allow_html=True)
    ...
    if camera_image:
        ...
        if confidence_score < 0.80:
            ...
            # Handle user context
            new_entry["username"] = current_user

            if os.path.exists(csv_file):
                df = pd.read_csv(csv_file)
            else:
                df = pd.DataFrame(columns=["username", "item", "conf", "time"])
            ...
            with btn_col2:
                ...
                target_page = "pages/about.py"
                if "battery" in label_key:
                    target_page = "pages/battery.py"
                elif "biological" in label_key or "organic" in label_key:
                    target_page = "pages/biological.py"
                elif "cardboard" in label_key:
                    target_page = "pages/cardboard.py"
                elif "glass" in label_key:
                    target_page = "pages/glass.py"
                elif "metal" in label_key:
                    target_page = "pages/metal.py"
                elif "paper" in label_key:
                    target_page = "pages/paper.py"
                elif "plastic" in label_key:
                    target_page = "pages/plastic.py"

                # Dynamic Button Text
                button_text = f"Read More about {clean_label}"
                
                if st.button(button_text, use_container_width=True):
                    try:
                        st.switch_page(target_page)
                    except Exception as e:
                        st.error(f"Could not open {target_page}. Going to About page.")
